  ...
  const formSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        document.cookie = `user-info=${encodeURIComponent(JSON.stringify(response.data))}; path=/;`;
      } else {
        toast.dismiss(toastId);
      ...
      toast.dismiss(toastId);
      toast.error(error.response?.data?.message || 'An error occurred. Please try again.', { duration: 2000 });
    } finally {
      toastId = null;
    }
  };
  ...
  return (
    <div className="min-h-screen flex justify-center items-center bg-gradient-to-r from-emerald-900 via-teal-700 to-emerald-900">
      ...
      <form onSubmit={formSubmit} className="bg-white shadow-md rounded-lg p-8 w-[400px]">
        ...
        <Input label="Email" placeholder="Enter your email" name="email" type="email" value={formData.email} onChange={inputChange} autoComplete="username" />
        <Input label="Password" placeholder="Enter your password" name="password" type="password" value={formData.password} onChange={inputChange} autoComplete="current-password" />
        <button type="submit" className="w-full bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 text-white font-bold py-2 px-4 rounded-lg transition duration-200">
          Log In
        </button>
